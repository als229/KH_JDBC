...
public class MemberView {
	...
	private void delectMember() {
		...
		System.out.print("삭제하고싶은 유저의 아이디 > ");
		String userId = sc.nextLine();
		System.out.print("당신 주인 맞아? 비밀번호 > ");
		String userPwd = sc.nextLine();

		if (mc.updateLoginId(userId, userPwd) == 1) {
			return;
		}
	...
	public void updateMember() {
		...
		while (true) {
			...
			if (mc.updateLoginId(inputUserId, inputUserPwd) == 1) {
				continue;
			}
			...
			// 회원 추가 요청 => Controller의 어떤 메서드 호출
			mc.updateMember(inputUserId, userId, userPwd, userName, gender, age, email, phone, address, hobby);
			break;
		}
	}
